...
if __name__ == '__main__':
    ...
    for i, station in enumerate(Stations):
        ...
        plt.plot(wavelength, transect[wavelength], color='tab:blue', marker='.')
        plt.xlabel('Distance along profile')
        plt.ylabel('Autocorrelation~[m$^{2}$]')
        plt.xlim(0, transect.size)
        plt.savefig('Analysis_DEM_' + station + '.pdf')
        np.save('Analysis_DEM_' + station + '.npy', {'orientation': orientation, 'wavelength': x_transect[wavelength], 'amplitude': amplitude})
