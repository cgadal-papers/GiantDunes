...
Data_coeff = np.load(os.path.join(path_outputdata, 'time_series_hydro_coeffs.npy'), allow_pickle=True).item()
Data_coeff = np.concatenate([Data_coeff[station] for station in Data_coeff.keys()], axis=1)
...
# %%
# Plotting the regime diagrams for A -- binned plots
# -------------------------------------------------------------

norm = mpcolors.TwoSlopeNorm(vcenter=0, vmax=70)
A_Coeff = Data_coeff[0, :]
log_counts_max = np.log10(2230)
...
for i, (var1, var2) in enumerate(couples):
    ...
    bin2 = np.logspace(np.floor(np.log10(numbers[var2][mask].min())), np.ceil(np.log10(numbers[var2][mask].max())), 50)
    bins = [bin1, bin2]
    a = plot_regime_diagram(ax, A_Coeff[mask], vars, lims_list, xlabel, ylabel, bins=bins, norm=norm, cmap='seismic', type='binned')

# #### colorbar
cb = fig.colorbar(a, cax=plt.subplot(gs[0]), orientation='horizontal')
cb.set_label(r'$\mathcal{A}_{0}$')
cb.ax.xaxis.set_ticks_position('top')
cb.ax.xaxis.set_label_position('top')

plt.savefig(os.path.join(path_savefig, 'regime_diagrams_binned_A_Coeff.pdf'))
plt.show()

# %%
# Plotting the regime diagrams for B -- binned plots
# -------------------------------------------------------------
norm = mpcolors.TwoSlopeNorm(vcenter=0, vmin=-70)
B_Coeff = Data_coeff[1, :]
log_counts_max = np.log10(2230)
...
for i, (var1, var2) in enumerate(couples):
    ...
    bin2 = np.logspace(np.floor(np.log10(numbers[var2][mask].min())), np.ceil(np.log10(numbers[var2][mask].max())), 50)
    bins = [bin1, bin2]
    a = plot_regime_diagram(ax, B_Coeff[mask], vars, lims_list, xlabel, ylabel, bins=bins, norm=norm, cmap='seismic', type='binned')

# #### colorbar
cb = fig.colorbar(a, cax=plt.subplot(gs[0]), orientation='horizontal')
cb.set_label(r'$\mathcal{B}_{0}$')
cb.ax.xaxis.set_ticks_position('top')
cb.ax.xaxis.set_label_position('top')

plt.savefig(os.path.join(path_savefig, 'regime_diagrams_binned_B_Coeff.pdf'))
plt.show()

# %%
# Plotting the regime diagrams for arctan(B/A) -- binned plots
# -------------------------------------------------------------
norm = mpcolors.TwoSlopeNorm(vcenter=0)
at = np.arctan2(Data_coeff[1, :], Data_coeff[0, :])
log_counts_max = np.log10(2230)
...
for i, (var1, var2) in enumerate(couples):
    ...
    bin2 = np.logspace(np.floor(np.log10(numbers[var2][mask].min())), np.ceil(np.log10(numbers[var2][mask].max())), 50)
    bins = [bin1, bin2]
    a = plot_regime_diagram(ax, at[mask], vars, lims_list, xlabel, ylabel, bins=bins, norm=norm, cmap='seismic', type='binned')

# #### colorbar
cb = fig.colorbar(a, cax=plt.subplot(gs[0]), orientation='horizontal')
cb.set_label(r'$\textup{arctan}(\mathcal{B}_{0}/\mathcal{A}_{0})$')
cb.ax.xaxis.set_ticks_position('top')
cb.ax.xaxis.set_label_position('top')

plt.savefig(os.path.join(path_savefig, 'regime_diagrams_binned_atan2.pdf'))
plt.show()
...
TAU = Cisaillement_basal(x, y, alphas_th, A0, B0, k*xi)
#
...
norms = [mpcolors.Normalize(vmin=0, vmax=50), mpcolors.TwoSlopeNorm(vmin=-1, vcenter=0, vmax=0.1)]
#
fig = plt.figure(figsize=(theme.fig_width, theme.fig_width))
