...
def _solve_system(eta_0, eta_H, eta_h0, Kappa=0.4, max_z=None, method='DOP853', dense_output=True, **kwargs):
    eta_span_tp = [eta_h0, eta_H] if max_z is None else [eta_h0, max_z]
    # eta_val = np.linspace(0, eta_H, 100)
    X0_vec = [np.array([-_mu_prime(eta_h0, eta_0, Kappa), 1j*mu(eta_h0, eta_0, Kappa), 0, 0], dtype='complex_'),
...
def calculate_solution(eta, eta_H, eta_h0, eta_0, eta_B, Fr, max_z, Kappa=0.4, output='simple', **kwargs):
