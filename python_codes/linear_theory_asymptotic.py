...
def _solve_system(eta_0, eta_H, eta_lim, Kappa=0.4, max_z=None, method='DOP853', dense_output=True, **kwargs):
    eta_span_tp = [eta_lim, eta_H] if max_z is None else [eta_lim, max_z]
    # eta_val = np.linspace(0, eta_H, 100)
    X0_vec = [np.array([-_mu_prime(eta_lim, eta_0, Kappa), 1j*mu(eta_lim, eta_0, Kappa), 0, 0], dtype='complex_'),
              np.array([mu(eta_lim, eta_0, Kappa)/2, -1j*eta_lim*mu(eta_lim, eta_0, Kappa)/2 + 1j*eta_lim/(2*Kappa), 1, 0], dtype='complex_'),
              np.array([1j*eta_lim/(2*Kappa), eta_lim**2/(2*Kappa), eta_lim, 1], dtype='complex_'),
...
def calculate_solution(eta, eta_H, eta_lim, eta_0, eta_B, Fr, max_z, Kappa=0.4, output='simple', **kwargs):
    ...
    """
    Results = _solve_system(eta_0, eta_H, eta_lim, Kappa=0.4, max_z=max_z, atol=1e-10, rtol=1e-10, **kwargs)
    # Results = _solve_system(eta_0, eta_H, eta_h0, Kappa=0.4, max_z=max_z, atol=1e-10, rtol=1e-10, **kwargs)
